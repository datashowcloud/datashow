...
//		}
    ...
    $('#tblGrid tbody').on('click', '.deletefase', function () {
		...
		if (result) {
			...
			var mygroup = child.eq(0).text();
			var mycode = child.eq(1).text();
			deletefase(mytema, mycategory, myid, mygroup, mycode);
...
async function deletefase(mytema, mycategory, myid, mygroup, mycode) {
    try {
...
async function initLinkHelp() {
	...
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, 'ECS', 'EKS', 'ECR', 'S3', save, 'EC2', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/concepts.html', 'computação escalável na Nuvem da Amazon Web Services (AWS)', 'O Amazon EC2 (Elastic Compute Cloud) --> Oferece uma capacidade de computação escalável na Nuvem da Amazon Web Services (AWS). O uso dele elimina a necessidade de investir em hardware inicialmente, portanto, você pode desenvolver e implantar aplicativos com mais rapidez.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'EC2', 'Instâncias Sob Demanda', 'Instâncias Spot', 'Budgets', save, 'Instâncias T2 no EC2', 'https://aws.amazon.com/pt/ec2/instance-types/t2/', 't2.nano (é a menor com 1 CPU e 0,5 RAM)', 'As instâncias T2 do serviço EC2 --> São um novo tipo de instância de uso geral de baixo custo. Detalhes: t2.nano (é a menor com 1 CPU e 0,5 RAM), t2.micro, t2.small, t2.medium, t2.large, t2.xlarge, t2.2xlarge (é a maior com 8 CPUs e 32 RAM).');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'EC2', 'Classes de armazenamento', 'EFS', 'EBS', save, 'S3', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/concepts.html', 'armazenamento de objetos que armazena dados como objetos em buckets (exemplo de arquivos estáticos: .html, .js, .cs', 'O Amazon S3 (Simple Storage Service) --> É um serviço de armazenamento de objetos que armazena dados como objetos em buckets (exemplo de arquivos estáticos: .html, .js, .cs.). A capacidade é virtualmente ilimitada. Um objeto é um arquivo e quaisquer metadados que descrevam o arquivo. Um bucket é um contêiner de objetos. Você pode controlar o acesso a grupos de objetos que começam com um prefixo ou termine com uma determinada extensão. É compatível com a replicação entre regiões.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'S3', 'IAM Policies', 'IAM Roles', 'IAM', save, 'S3 pre-signed URL', 'https://docs.aws.amazon.com/pt_br/AmazonS3/latest/userguide/ShareObjectPreSignedURL.html', '', 'O pre-signed URL --> Permite acessar objetos do S3 embora, por padrão, todos os objetos do S3 são privados. Somente o proprietário do objeto tem permissão para acessá-lo. Contudo, também permite o proprietário do objeto compartilhar objetos com os outros dessa forma, usando suas próprias credenciais de segurança para conceder permissão de prazo limitado para download de objetos.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'EC2', 'S3', 'Instâncias Spot', 'Instâncias Sob Demanda', save, 'Opções de Compra de Instância', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/instance-purchasing-options.html', '', '<b>On-demand</b> (Instâncias sob demanda), paga somente pelo uso.<br/><b>Savings Plans - Planos de Poupança</b> é um modelo de preços flexíveis que oferece preços mais baixos em comparação com os preços sob demanda, em troca de um compromisso de uso específico por um período de 1 ou 3 anos)<br/><b> Instâncias Reservadas</b> (Instâncias reservadas), paga por um contrato pré-estabelecido de 1 ou 3 anos de compromisso para obter um desconto até 72% comparado ao On-Demand.<br/><b>Instâncias Spot</b> (Instâncias spot), paga pelo uso dos recursos não utilizados por outros modelos.<br/><b>Dedicated Hosts</b> (Hosts dedicados).<br/><b>Dedicated Instances</b> (Instâncias dedicadas).<br/><b>Capacity Reservations</b> (Reservas de Capacidade).');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'Instâncias Spot', 'Instâncias Sob Demanda', 'Instâncias Reservadas', 'Budgets', save, 'Savings Plans', 'https://aws.amazon.com/pt/savingsplans/', '', 'é um modelo de preços flexíveis que oferece preços mais baixos em comparação com os preços sob demanda, em troca de um compromisso de uso específico por um período de 1 ou 3 anos');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'Savings Plans', 'Instâncias Sob Demanda', 'Instâncias Spot', 'S3', save, 'Instâncias Reservadas', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/ec2-reserved-instances.html', '', 'As instâncias reservadas --> Proporcionam economia significativa em seus custos do Amazon EC2 em comparação com os preços de instâncias sob demanda. Elas não são instâncias físicas, é um desconto na fatura aplicado na conta pelo uso de instâncias sob demanda. O cliente paga por um contrato pré-estabelecido de 1 ou 3 anos de compromisso para obter um grande desconto. Reserva capacidade a uma taxa com desconto. O cliente se compromete a comprar uma certa quantidade de computação.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'Savings Plans', 'Instâncias Spot', 'Instâncias Reservadas', 'Budgets', save, 'Instâncias Sob Demanda', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/ec2-on-demand-instances.html', '', 'Instâncias sob demanda --> Você paga somente pelo uso e pela capacidade computacional por segundo, sem qualquer compromisso de longo prazo. Não há compromisso de longo prazo ao comprá-la. Você paga apenas pelos segundos em que essas instâncias estiverem no estado running, com um mínimo de 60 segundos.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup+'', contadorMycode+'', contadorMycode+'', 'Savings Plans', 'Instâncias Reservadas', 'Instâncias Sob Demanda', 'EC2', save, 'Instâncias Spot', 'https://aws.amazon.com/pt/ec2/spot/', '', 'As instâncias spot do Amazon EC2 --> Permitem aproveitar a capacidade não utilizada do EC2 na Nuvem AWS. Em comparação com a definição de preço sob demanda, essas instâncias oferecem descontos de até 90%. Se ajusta com base na oferta e na demanda de instâncias EC2.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, 'S3', 'EC2', 'EFS', 'EBS', save, 'Modelo de uso', 'https://aws.amazon.com/pt/free/free-tier-faqs/', 'composto por três tipos', 'Modelo de uso --> O nível gratuito da AWS oferece aos clientes a capacidade de explorar e testar gratuitamente serviços da AWS até os limites especificados para cada serviço. O nível gratuito é composto por três tipos diferentes de ofertas: um nível gratuito de 12 meses, uma oferta Always Free e testes de curto prazo.');
...
var save = false; //não precisa gravar o restante na tabela porque estão nos arquivos, exemplo: T1C1G11.html
	...
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, '', '', '', '', save, 'Acoplamento fraco', 'https://wa.aws.amazon.com/wat.question.REL_4.pt_BR.html', 'uma das melhores práticas é o baixo acoplamento', 'No pilar de Confiabilidade do AWS Well-Architected Framework, uma das melhores práticas é o baixo acoplamento ajuda a isolar o comportamento de um componente dos outros componentes que dependem dele, o que aumenta a resiliência e a agilidade. Uma falha em um dos componentes não deve afetar os outros componentes.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, '', '', '', '', save, 'Elasticidade', 'https://aws.amazon.com/pt/what-is-cloud-computing/?nc2=h_ql_le_int_cc', 'provisiona a quantidade de recursos realmente necessária', 'A capacidade de adquirir recursos quando você precisa deles e liberá-los quando você não precisar mais. Com a computação em nuvem, você não precisa provisionar recursos em excesso para absorver picos de atividades empresariais no futuro. Em vez disso, você provisiona a quantidade de recursos realmente necessária.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	...
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, '', '', '', '', save, 'SDK', 'https://aws.amazon.com/pt/developer/tools/', 'C++, Go, Java, JavaScript, Kotlin, .NET, Node.js, PHP, Python, Ruby, Rust, Swift', 'Desenvolver aplicativos na AWS na linguagem de programação de sua escolha, como: C++, Go, Java, JavaScript, Kotlin, .NET, Node.js, PHP, Python, Ruby, Rust, Swift.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	...
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, '', '', '', '', save, 'Snow', 'https://aws.amazon.com/pt/snow/', 'Migrar petabytes de dados', 'Dispositivos desenvolvidos especificamente para migrar petabytes de dados de forma econômica, offline para a AWS ou processar dados na borda. Dispositivos da família: AWS Snowcone ou AWS Snowball.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
	...
	contadorMycode = String(parseInt(contadorMycode) + 1);
	linkhelp = linkhelp + getLinkHelp(mytema, mycategory, contadorMygroup, contadorMycode, contadorMycode, '', '', '', '', save, 'Detective', 'https://aws.amazon.com/pt/detective/faqs/', 'facilita a análise, a investigação e a identificação rápidas da causa raiz de potenciais problemas de segurança ou atividades suspeitas', 'O Amazon Detective facilita a análise, a investigação e a identificação rápidas da causa raiz de potenciais problemas de segurança ou atividades suspeitas. O Amazon Detective coleta automaticamente dados de log de seus recursos da AWS e usa aprendizado de máquina, análise estatística e teoria dos gráficos para criar um conjunto de dados vinculados que permite realizar facilmente investigações de segurança mais rápidas e eficientes.');
	contadorMycode = String(parseInt(contadorMycode) + 1);
