...
//		refreshMenu(mytema, mycategory, '0', '', '', '');
//		showFormApresentacao();
	refreshMenu(mytema, mycategory, '0', '', '', '');
	refreshTableData(mytema, mycategory, '0', '', '', '');
	showGridAndHideForms();
		...
		students.forEach(function (student) {
			htmlString += '<label align=center style="ZZZwidth:290px; border-radius:10px; border-style:solid; border-color:white; border-width:0px; color:#000000; background-color:transparent;">';
				htmlString += "&nbsp;<label ItemId=" + student.id + '>';
					...
					htmlString += '</p>';
					htmlString += '&nbsp;';
					htmlString += '<a href=\"#\" ' + varClass + ' style="color:white; font-size:18px; border-radius:10px;" onclick=\"refreshTableQuestion(' + mytema + ', ' + mycategory + ', ' + student.id + ', ' + student.mygroup + ', 1);';
					htmlString += ' showFormSim(' + mytema + ', ' + mycategory + ', ' + student.mygroup + '); \">';
						htmlString += '<b>' + varPlay + '</b>';
						htmlString += '<br/>';
						//texto
					...
					htmlString += '<button class="flutuante" onclick="refreshFase(' + mytema + ', ' + mycategory + ', ' + student.mygroup + ', ' + student.mycode + ', ' + student.id + ')" style="border-radius:50%; position:relative; top:-5px; border:2px solid gray;" title="Baixar novidades"> <i class="fa fa-refresh"></i> </button>';
					htmlString += '<br/>';
				htmlString += '</label>';
			htmlString += '</label> &nbsp;&nbsp;';
		...
		divModoDesafio += '<div align=center style="border-radius:2.18vw; margin-left:auto; margin-right:auto;">';
			...
			divModoDesafio += '<label align=center class="degradeblue" style="width:95%; border-radius:10px; border-style:solid; border-color:yellow; border-width:3px; color:#000000; background-color:transparent;">';
				...
				divModoDesafio += '<a href=\"#\" class="btn btn-default degradeblack" style="color:white; font-size:18px; border-radius:10px;">';
					divModoDesafio += '<b>BLOQUEADO</b>';
					divModoDesafio += '<br/><b>FASE 1</b>';
					divModoDesafio += '<br/>0%';
				divModoDesafio += '</a>';
				divModoDesafio += '<br/><br/>';
			divModoDesafio += '</label>';
		divModoDesafio += '</div>';
		
		var divRodape = '<br/><br/><br/><br/><br/>';
		
        $('#tblMinhasConquistas').html(divModoTreino + divModoDesafio + divRodape);
...
//This function refreshes the table
async function refreshMenu(mytema, mycategory, mycode, myorder, mygroup, mytext, mytext2, mytext3) {
	var students = await jsstoreCon.select({
		from: 'Student'
			, where: { mytema: mytema + ''
			, mycategory: mycategory + ''
...
//	setConfigBotoes(); //atualiza o destaque de cor dos botões de modo de jogo
	...
	htmlString += '<div align=center style="border-radius:2.18vw; margin-left:auto; margin-right:auto;">';
		...
		htmlString += '<label align=center class="degradeblue" style="width:95%; border-radius:10px; border-style:solid; border-color:yellow; border-width:3px; color:#000000; background-color:transparent;">';
			...
			htmlString += '<button id="btnTest" style="display:none;">está aqui para tirar o foco ao clicar no painel label</button>';
			htmlString += '<label class="flutuante" style="border-radius:30px; border-style:double; border-color:white; border-width:2px; color:#000000; background-color:#FFFFFF;">';
				htmlString += '<button id="btnNivel" class="btn btn-success degradegreen" onclick="refreshTableQuestion(' + mytema + ', ' + mycategory + ', ' + myid + ', ' + mygroup + ', ' + mycode + '); showFormSim(' + mytema + ', ' + mycategory + ', ' + mygroup + ');" style="border-radius:30px; font-size:30px; font-family:Helvetica; font-weight:bold; border-width:0px; -webkit-text-stroke-width: 1px; ">';
					htmlString += '&nbsp;&nbsp; COMEÇAR &nbsp;&nbsp;';
				htmlString += '</button>';
			htmlString += '</label>';
			...
			htmlString += '<div style="width:98%;  margin-left:auto; margin-right:auto;" align=center>';

				htmlString += '<a href="#topMinhasConquistas">';
					htmlString += '<label class="flutuante" style="border-radius:5px; border-style:double; border-color:white; border-width:2px; color:#000000; background-color:#FFFFFF;">';
						htmlString += '<button id="btnMinhasConquistas" class="btn btn-primary degradeblue"><i class="fa fa-list" style="font-size:35px;" title="Lista de fases e níveis"></i></button>';
					htmlString += '</label>';
				htmlString += '</a>';
				htmlString += '<br/><font size=1 style="color:white;">Minhas Conquistas</font>';
