...
function initLinkHelp() {
	...
	linkhelp = linkhelp + getLinkHelp('Não Se Aplica', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/concepts.html', 'Não', 'Não Se Aplica na AWS');
	...
	linkhelp = linkhelp + getLinkHelp('S3', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/concepts.html', 'armazenamento de objetos que armazena dados como objetos em buckets (exemplo de arquivos estáticos: .html, .js, .cs', 'O Amazon S3 (Simple Storage Service) é um serviço de armazenamento de objetos que armazena dados como objetos em buckets (exemplo de arquivos estáticos: .html, .js, .cs.). Um objeto é um arquivo e quaisquer metadados que descrevam o arquivo. Um bucket é um contêiner de objetos. Você pode controlar o acesso a grupos de objetos que começam com um prefixo ou termine com uma determinada extensão.');
	...
	linkhelp = linkhelp + getLinkHelp('Storage Gateway', 'https://aws.amazon.com/pt/storagegateway/', '', 'AWS Storage Gateway é um conjunto de serviços de armazenamento na nuvem híbrida que oferece acesso on-premises para armazenamento virtual na nuvem praticamente ilimitado.');
	...
	linkhelp = linkhelp + getLinkHelp('DMS', 'https://aws.amazon.com/pt/dms/', '', 'AWS DMS (Database Migration Service), ajuda você a migrar bancos de dados para a AWS de modo rápido e seguro. O banco de dados de origem permanece totalmente operacional durante a migração, minimizando o tempo de inatividade de aplicações que dependem do banco de dados.');
	...
	linkhelp = linkhelp + getLinkHelp('EMR', 'https://aws.amazon.com/pt/emr/', '', 'AWS EMR (Amazon Elastic MapReduce) é uma plataforma de <b>Big Data>/b> em nuvem usada para executar trabalhos de processamento de dados distribuídos em grande escala, consultas SQL interativas e aplicações de machine learning (ML) usando frameworks de análise de código aberto como Apache Spark, Apache Hive e Presto.');
	...
	linkhelp = linkhelp + getLinkHelp('EventBridge', 'https://aws.amazon.com/pt/eventbridge/?nc2=h_ql_prod_ap_eb', '', 'O Amazon EventBridge é um barramento de eventos sem servidor que torna mais fácil a criação de aplicações orientadas por eventos em escala usando eventos gerados com base em suas aplicações, aplicações integradas de software como serviço (SaaS) e serviços da AWS.');
	...
	linkhelp = linkhelp + getLinkHelp('IAAS', 'https://aws.amazon.com/pt/what-is-cloud-computing/?nc2=h_ql_le_int_cc', '', 'IaaS (Infraestrutura como Serviço) contém os componentes básicos da IT na nuvem. Normalmente, o IaaS oferece acesso a recursos de rede, computadores (virtuais ou em hardware dedicado) e espaço de armazenamento de dados. O IaaS oferece o mais alto nível de flexibilidade e controle de gerenciamento sobre os recursos de infraestrutura de tecnologia.');
	linkhelp = linkhelp + getLinkHelp('SAAS', 'https://aws.amazon.com/pt/what-is-cloud-computing/?nc2=h_ql_le_int_cc', '', 'SaaS (Software como Serviço) O SaaS oferece um produto completo, executado e gerenciado pelo provedor de serviços. Na maioria dos casos, refere-se a aplicativos de usuários finais (como e-mail baseado na web). Com uma oferta de SaaS, você não precisa pensar sobre a manutenção do serviço ou o gerenciamento da infraestrutura subjacente. Você só precisa se preocupar sobre como utilizará esse software específico.');
	linkhelp = linkhelp + getLinkHelp('BAAS', 'https://aws.amazon.com/pt/what-is-cloud-computing/?nc2=h_ql_le_int_cc', '', 'BAAS (Backend As A Service) é um serviço que automatiza o desenvolvimento do backend, por meio da terceirização dessas funções. Pode ser classificado como um middleware, ou seja, um software que fornece serviços para outros aplicativos ou o próprio sistema. O backend é a estrutura que possibilita a operação do sistema.');
	linkhelp = linkhelp + getLinkHelp('PAAS', 'https://aws.amazon.com/pt/what-is-cloud-computing/?nc2=h_ql_le_int_cc', '', 'PaaS (Plataforma como Serviço) você não precisa mais gerenciar a infraestrutura subjacente (geralmente, hardware e sistemas operacionais) e pode manter o foco na implantação e no gerenciamento de aplicativos.');
	linkhelp = linkhelp + getLinkHelp('Cognito', 'https://docs.aws.amazon.com/pt_br/cognito/latest/developerguide/what-is-amazon-cognito.html', '', 'AWS Cognito permite adicionar cadastramento, login e controle de acesso de usuários a aplicações Web e móveis com rapidez e facilidade para vários dispositivos.');
	linkhelp = linkhelp + getLinkHelp('IAM', 'https://aws.amazon.com/pt/iam/', '', 'AWS IAM (Identity and Access Management) fornece controle de acesso refinado em toda a AWS. Com o IAM, é possível especificar quem pode acessar quais serviços e recursos e em que condições. Com as políticas do IAM, você gerencia permissões para seu quadro de funcionários e sistemas para garantir permissões com privilégios mínimos.');
