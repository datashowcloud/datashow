...
function initLinkHelp() {
	...
	linkhelp = linkhelp + getLinkHelp('S3 pre-signed URL', 'https://docs.aws.amazon.com/pt_br/AmazonS3/latest/userguide/ShareObjectPreSignedURL.html', 'outros podem acessar somente com um pre-signed URL', 'Por padrão, todos os objetos do S3 são privados e outros podem acessar somente com um pre-signed URL. Somente o proprietário do objeto tem permissão para acessá-lo. Contudo, o proprietário do objeto pode compartilhar objetos com os outros criando um pre-signed URL, usando suas próprias credenciais de segurança para conceder permissão de prazo limitado para download de objetos.');
	linkhelp = linkhelp + getLinkHelp('Instance Purchasing Options', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/instance-purchasing-options.html', '', '<b>On-demand</b> (Instâncias sob demanda), paga somente pelo uso.<br/><b>Savings Plans</b> é um modelo de preços flexíveis que oferece preços mais baixos em comparação com os preços sob demanda, em troca de um compromisso de uso específico por um período de 1 ou 3 anos)<br/><b> Reserved Instances</b> (Instâncias reservadas), paga por um contrato pré-estabelecido de 1 ou 3 anos de compromisso para obter um desconto até 72% comparado ao On-Demand.<br/><b>Spot Instances</b> (Instâncias spot), paga pelo uso dos recursos não utilizados por outros modelos.<br/><b>Dedicated Hosts</b> (Hosts dedicados).<br/><b>Dedicated Instances</b> (Instâncias dedicadas).<br/><b>Capacity Reservations</b> (Reservas de Capacidade).');
	linkhelp = linkhelp + getLinkHelp('Reserved Instances', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/ec2-reserved-instances.html', '', 'As instâncias reservadas proporcionam economia significativa em seus custos do Amazon EC2 em comparação com os preços de instâncias sob demanda. As instâncias reservadas não são instâncias físicas, é um desconto na fatura aplicado na conta pelo uso de instâncias sob demanda. O cliente paga por um contrato pré-estabelecido de 1 ou 3 anos de compromisso para obter um grande desconto. Reserva capacidade a uma taxa com desconto. O cliente se compromete a comprar uma certa quantidade de computação.');
	linkhelp = linkhelp + getLinkHelp('On-Demand Instances', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/ec2-on-demand-instances.html', '', 'Instâncias sob demanda, você paga somente pelo uso e pela capacidade computacional por segundo, sem qualquer compromisso de longo prazo. Não há compromisso de longo prazo ao comprar Instâncias on-demand. Você paga apenas pelos segundos em que suas instâncias sob demanda estiverem no estado running, com um mínimo de 60 segundos.');
	linkhelp = linkhelp + getLinkHelp('Spot Instances', 'https://aws.amazon.com/pt/ec2/spot/', '', 'As instâncias spot do Amazon EC2 permitem aproveitar a capacidade não utilizada do EC2 na Nuvem AWS. Em comparação com a definição de preço sob demanda, as instâncias spot oferecem descontos de até 90%. Se ajusta com base na oferta e na demanda de instâncias EC2.');
	...
	linkhelp = linkhelp + getLinkHelp('Organizations', 'https://aws.amazon.com/pt/organizations/?nc2=type_a', 'alocar recursos, agrupar contas', 'AWS Organizations para criar novas contas da AWS e alocar recursos, agrupar contas para organizar seus fluxos de trabalho, aplicar políticas a contas ou grupos para governança e simplificar o faturamento usando <b>um único método de pagamento</b> para todas as suas contas qualificando preços por volume.');
	...
	linkhelp = linkhelp + getLinkHelp('Lambda', 'https://docs.aws.amazon.com/pt_br/lambda/latest/dg/welcome.html', '', 'AWS Lambda é um serviço de computação que permite executar código sem provisionar ou gerenciar servidores, ou seja, processa dados sem servidor e pode ser acionado pelo S3 e SNS.');
	...
	linkhelp = linkhelp + getLinkHelp('Business', '', '', '');
	...
	linkhelp = linkhelp + getLinkHelp('IAM', 'https://aws.amazon.com/pt/iam/', 'controle de acesso a serviços e recursos', 'AWS IAM (Identity and Access Management) fornece controle de acesso a serviços e recursos em determinada condição em toda a AWS. Com as políticas do IAM, você gerencia permissões para seu quadro de funcionários e sistemas para garantir permissões com privilégios mínimos no princípio do menor privilégio. O IAM não alerta sobre eventos de login do console por usuário raiz.');
	...
	linkhelp = linkhelp + getLinkHelp('CloudWatch', 'https://aws.amazon.com/pt/cloudwatch/faqs/', '', 'AWS CloudWatch, é um serviço de monitoramento para recursos em nuvem AWS e os aplicativos que você executa na AWS. Você pode usar para coletar e rastrear métricas, coletar e monitorar arquivos de log, e definir alarmes de faturamento para monitorar as cobranças estimadas da AWS.');
	...
	linkhelp = linkhelp + getLinkHelp('Elasticidade', 'https://aws.amazon.com/pt/what-is-cloud-computing/?nc2=h_ql_le_int_cc', 'provisiona a quantidade de recursos realmente necessária', 'A capacidade de adquirir recursos quando você precisa deles e liberá-los quando você não precisar mais. Com a computação em nuvem, você não precisa provisionar recursos em excesso para absorver picos de atividades empresariais no futuro. Em vez disso, você provisiona a quantidade de recursos realmente necessária.');
	...
	linkhelp = linkhelp + getLinkHelp('CloudFormation', 'https://aws.amazon.com/pt/cloudformation/', '', 'Permite modelar, provisionar e gerenciar recursos da AWS e de terceiros ao tratar a infraestrutura como código. Permite padronizar e versionar as configurações de banco de dados. de forma automatizada usando pipelines de CI/CD.');
	...
	linkhelp = linkhelp + getLinkHelp('NACL', 'https://docs.aws.amazon.com/pt_br/vpc/latest/userguide/vpc-network-acls.html', 'tráfego de entrada ou de saída no nível da sub-rede', 'Uma lista de controle de acesso (ACL) de rede permite ou não determinado tráfego de entrada ou de saída no nível da sub-rede (subnet).');
