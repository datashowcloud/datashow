...
function initLinkHelp() {
	...
	linkhelp = linkhelp + getLinkHelp('Reserved Instances', 'https://docs.aws.amazon.com/pt_br/AWSEC2/latest/UserGuide/ec2-reserved-instances.html', '', 'As instâncias reservadas proporcionam economia significativa em seus custos do Amazon EC2 em comparação com os preços de instâncias sob demanda. As instâncias reservadas não são instâncias físicas, é um desconto na fatura aplicado na conta pelo uso de instâncias sob demanda. O cliente paga por um contrato pré-estabelecido de 1 ou 3 anos de compromisso para obter um grande desconto.');
	...
	linkhelp = linkhelp + getLinkHelp('Spot Instances', 'https://aws.amazon.com/pt/ec2/spot/', '', 'As instâncias spot do Amazon EC2 permitem aproveitar a capacidade não utilizada do EC2 na Nuvem AWS. Em comparação com a definição de preço sob demanda, as instâncias spot oferecem descontos de até 90%.');
	...
	linkhelp = linkhelp + getLinkHelp('EBS', 'https://aws.amazon.com/pt/ebs/', '', 'EBS (Amazon Elastic Block Store) é um serviço de armazenamento em blocos, escalável e de alta performance projetado para o Amazon Elastic Compute Cloud (Amazon EC2). É necessário associar um EBS a uma instância EC2 para garantir a persistência dos dados quando a instância é desligada. Essa arquitetura de banco de dados pode ser gerenciada pelo time do Cliente.');
